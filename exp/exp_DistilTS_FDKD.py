...
class Exp_DistilTS(Exp_Basic):
    ...
    def _rfft_log_mag(self, x: torch.Tensor, eps: float) -> torch.Tensor:
        X = torch.fft.rfft(x.float(), dim=1)
        mag = torch.abs(X).clamp_min(eps)
        return torch.log(mag)

    def _freq_kd_loss(self,
                    student_y: torch.Tensor,
                    teacher_y: torch.Tensor,
                    eps: float,
                    beta: float,
                    diff_gamma: float) -> torch.Tensor:

        # 1) 频域（对数幅度谱）
        s_logmag = self._rfft_log_mag(student_y, eps)   # [B, F, C]
        t_logmag = self._rfft_log_mag(teacher_y, eps)   # [B, F, C]
        l_freq = torch.mean((s_logmag - t_logmag) ** 2)

        # 2) 时域一阶差分（保持局部变化形状，但不强制逐点对齐）
        if diff_gamma > 0:
            s_diff = student_y[:, 1:, :] - student_y[:, :-1, :]
            t_diff = teacher_y[:, 1:, :] - teacher_y[:, :-1, :]
            l_diff = torch.mean((s_diff - t_diff) ** 2)
        else:
            l_diff = torch.tensor(0.0, device=student_y.device)

        return beta * l_freq + diff_gamma * l_diff
                    ...
                    with torch.cuda.amp.autocast():
                        ...
                        if self.teacher is not None and self.kd_alpha > 0.0:
                            with torch.no_grad():
                                ...
                                t_outputs = t_outputs[:, -self.args.pred_len:, f_dim:].float()
                            
                            kd_loss = self._freq_kd_loss(
                                student_y=outputs,
                                teacher_y=t_outputs,
                                eps=float(self.args.kd_freq_eps),
                                beta=float(self.args.kd_freq_beta),
                                diff_gamma=float(self.args.kd_freq_diff_gamma),
                            )
